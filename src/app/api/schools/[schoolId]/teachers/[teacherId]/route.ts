...
export async function PATCH(
  request: Request,
  props: { params: Promise<{ schoolId: string; teacherId: string }> }
) {
  try {
    const { schoolId, teacherId } = await props.params
    ...
    if (Object.keys(updates).length > 0) {
      ...
      if (!data) throw new ApiError('Failed to update teacher profile', 500)
      profile = data
    } else {
...
export async function DELETE(
  ...
  try {
    ...
    if (error) {
      throw new ApiError(error.message || 'Failed to delete teacher', 500)
    }
